...
Este patrón se repite "N" veces por cada id en cada cámara según la cantidad de veces que re-aparecieron. Cada captura está almacenada como un archivo \file{.png} de tamaño variable (aproximadamente 200 x 100 píxeles). En el nombre de la imagen se encuentra el número de frame para el cual fue \textcolor{red}{tomado dicha captura}. 
...
El entrenamiento fino partió del uso del modelo entrenado en la sección anterior. Esta vez se entrenó todas las capas del modelo utilizando una tasa de aprendizaje más baja a fin de realizar un ajuste fino, como se observa en la figura \ref{fig:entrenamientoFino}.
...
En la figura \ref{fig:comparacionExtractores} se observa que el modelo original de Osnet obtuvo mejor resultado que Cosnet. Cosnet supera los resultados obtenidos del modelo DeepMar, y por lo tanto, \textcolor{red}{el proceso de entrenamiento y pre-procesamiento fueron un éxito}.
...
El modelo Osnet tuvo un mejor desempeño porque fue entrenado con la función de pérdida \textit{triple-loss}, que maximiza la distancia entre clusters distintos. En un futuro trabajo \textcolor{red}{se continuará trabajando} sobre el entrenamiento de Cosnet con triple-loss.
...
A fin representar \textcolor{red}{a las personas} como un conjunto de datos, se utiliza el modelo extractor de características que produce un vector por cada persona. Los vectores, de 512 dimensiones, se utilizan para segmentar a las personas y transformarlas en clusters.

En la figura \ref{fig:comparacionExtractores} se observa como las capturas de una persona forman un cluster, en el cual se agrupan sus características, formando así un cluster por persona. Se utilizó la técnica de \textit{Nearest Centroid} \citep{NEAREST_CENTROID} para calcular el centro de cada cluster dado el conjunto de vectores que lo conforman.
...
\textcolor{red}{Finalmente}, cada cluster formado en el sistema representará a una persona con un identificador único (cluster id). A medida que la persona incorpora más vectores a su cluster más se robustece y mejor la representa.
...
Toda la aplicación funciona dentro del microframework de Flask, en donde, se captura las solicitudes de usuario por \textit{API REST (Representational 
